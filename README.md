...
The `runme.jl` script builds the rules, runs the simulation, generates ECA images and saves the image of the last generation of each rule to the `myimages/part-I-images` directory. The `testme.jl` script checks the validity of your image library against the expected images stored in the `testimages/part-I-images` directory.
